...
<xsl:template match="/">
	...
	<xsl:variable name="listName" select="'popups'" />
	...
	<xsl:variable name="srcXmlAggr" select="
		...
		<xsl:variable name="nodeId" select="lower-case(ywc:getParam('id', $params))" />
			
		<xsl:for-each select="$srcXmlAggr[@ows_ID= $nodeId]">
				
				<xsl:call-template name="ywcHeader">
					<xsl:with-param name="preUri" select="ywc:getParam('preUri', $params)"/>
					<xsl:with-param name="text" select="normalize-space(lower-case(@ows_Title))" />
					<xsl:with-param name="width" select="120"/>
					<xsl:with-param name="fontsize" select="24"/>
					<xsl:with-param name="color" select="$uiHeaderColor"/>
					<xsl:with-param name="bg-color" select="$uiHeaderBgColor"/>
				</xsl:call-template>
				
				<div style="position:relative;clear:both;margin-top:20px;width:90%;margin-left:5%;overflow:hidden;border:none;">
