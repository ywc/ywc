  ...
  <xsl:variable name="ywcPublicUri" select="if (string-length($ywcPublicCdn) &gt; 0) then concat('//',$ywcPublicCdn,'/') else concat($preUri,'public/')" />
	...
	<div class="ywc-crnr-5 ywc-crnr-t-off ywc-container-list-controls">
		...
		<xsl:if test="$showBttnAdd = 1">
			<img src="{$ywcPublicUri}ywc-image/bttn/misc/add-item-01.png" class="bttn-add-item" onClick="YWC.f.intranetCheckAuth(function(){'{'}YWC.f.intranetPopupPostEdit('{$listName}');{'}'});" onLoad="YWC.f.uiSetHoverBulge(this,10,'hz',true)" />
		</xsl:if>
		...
		<div class="asset-list-search {
			...
			<xsl:call-template name="ywcInputText">
				...
				<xsl:with-param name="eraseBttnJs" select='concat(
						...
						,");")'/>
				<xsl:with-param name="icon" select="concat($ywcPublicUri,'ywc-image/bttn/misc/search-01.png')"/>
		...
		<xsl:if test="$showBttnArchive = 1">
			<img src="{$ywcPublicUri}ywc-image/bttn/misc/books-01.png" class="bttn-archive {
		...
		<xsl:if test="$showBttnSubscribe = 1">
			<img src="{$ywcPublicUri}ywc-image/bttn/mail/envelope-01.png" class="bttn-subscribe" id="ywc-list-controls-bttn-subscribe-{$listName}" onClick="YWC.f.intranetCheckAuth(function(){'{'}YWC.f.intranetSubscribePopup('{$listName}');{'}'});" onLoad="YWC.f.uiSetHoverBulge(this,4,'hz',true)" />
		</xsl:if>
